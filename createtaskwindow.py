...
from utils import load_config, save_config, validate_alarm, validate_alarm_advance
import re
...
class CreateTaskWindow(tk.Toplevel):
  def __init__(self) -> None:
    ...
    self.taskAlarmLabel = ttk.Label(self, text='Horário:').grid(row=3, column=0, padx=(10, 5), pady=(5, 0), sticky='w')
    ...
    self.taskAlarmEntry.grid(row=3, column=1, columnspan=2, padx=(5, 10), pady=(5, 0), sticky='w')
    ...
    self.taskAlarmAdvanceCheckbox.grid(row=4, column=0, columnspan=3, padx=(10, 5), pady=(20, 0), sticky='w')
    ...
    self.taskAlarmAdvanceLabel1.grid(row=5, column=0, padx=(10, 0), pady=(5, 0), sticky='e')
    ...
    self.taskAlarmAdvanceEntry.grid(row=5, column=1, padx=5, pady=(5, 0), sticky='w')
    self.taskAlarmAdvanceLabel2 = ttk.Label(self, text='dias antes', state='disabled')
    self.taskAlarmAdvanceLabel2.grid(row=5, column=2, padx=(0, 5), pady=(5, 0), sticky='w')

    self.createTaskButton = ttk.Button(self, text='Criar Tarefa', command=self.save_new_task).grid(row=6, column=0, padx=(10, 5), pady=(15, 10), sticky='w')
    self.cancelButton = ttk.Button(self, text='Cancelar', command=self.destroy).grid(row=6, column=1, columnspan=2, padx=(0, 10), pady=(15, 10), sticky='w')
